...
import { default_libraries } from "@core/runtime/built-in/libraries";
import Environment, {
  createGlobalEnvironment,
} from "@core/runtime/environments";
import { evaluate } from "@core/runtime/interpreter";

import Parser from "@core/frontend/parser";

import { handleError, ImportError } from "@core/utils/errors_handler";
...
// Library Object Type
type Object = {
  functions: Record<string | number | symbol, never>;
  constants: Record<string | number | symbol, never>;
};
...
export async function compileLibrary(filePath: string, env: Environment) {
  ...
  if (filePath in default_libraries) {
    for (const [key, value] of Object.entries(default_libraries))
      if (key == filePath) library = value;
  } else {
    const fullPath = (process.cwd() + "\\" + filePath).replaceAll("\\", "/");
    if (!existsSync(fullPath))
      throw handleError(new ImportError("Library not found", filePath), 0, 0);

    switch (filePath.split(".")[1]) {
      case "gh3lib": {
        // Tries to import the library from the given file path
        try {
          library = (await import(fullPath)).default;
        } catch (error) {
      ...
      case "gh3": {
        ...
        await evaluate(await parser.produceAST(code, newEnv), newEnv);

        const exported = new Map<string, RuntimeValue>();
        for (const key of newEnv.exported.keys())
          exported.set(key, newEnv.lookupVar(key));

        let splits = filePath.split(".");
        splits = splits[splits.length - 2].split("/");
        return [
          { name: splits[splits.length - 1], object: MK_OBJECT(exported) },
        ];
      }
      default:
        throw handleError(
          new ImportError(
            "Library file must be of type 'Gh3sp Library' (.gh3lib)",
            fullPath,
          ),
          0,
          0,
  ...
  for (const [key, value] of Object.entries(library)) {
    ...
    if (!(value as Object).constants || !(value as Object).functions)
      throw handleError(
        new ImportError("Library doesn't contain a valid export", filePath),
        0,
        0,
      );
    ...
    for (const [f_key, f_value] of Object.entries((value as Object).functions))
      properties.set(f_key, MK_NATIVE_FUNCTION(f_value as FunctionCall));
    // Adds constants's functions to the properties map
    for (const [c_key, c_value] of Object.entries((value as Object).constants))
      properties.set(c_key, c_value as RuntimeValue);
