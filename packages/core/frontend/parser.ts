...
export default class Parser {
    ...
    switch (this.at().type) {
      ...
      case TokenType.Import:
        throw this.throwError(
          new ParserError(
            "Libraries can only be imported at the start of the program.",
          ),
        );
  ...
  private parse_function_declaration(): Statement {
    ...
    if (this.at().type == TokenType.Identifier) {
      ...
      if (this.isDeclaring)
        this.throwError(
          new SyntaxError(
            "Cannot declare a named function during variable declaration",
          ),
        );
      name = this.eat().value;
    }
    ...
    for (const arg of args) {
      ...
      if (arg.kind != "Identifier")
        this.throwError(
          new SyntaxError(
            "Expected string parameters inside of class declaration. " + arg,
          ),
        );
    ...
    // Throws an error if the function declaration isn't followed by a '{' or a '=>' token
      ...
      this.throwError(
        new SyntaxError("Expected '{' of '=>' after function declaration"),
      );
  ...
  private parse_variable_declaration(): Statement {
    ...
    if (
      ...
      if (isConstant)
        this.throwError(
          new SyntaxError(
            `Must assign value to constant expression '${assignee}'. No value provided.`,
          ),
        );
  ...
  private parse_class_declaration(): Statement {
    ...
    for (const arg of args) {
      ...
      if (arg.kind != "Identifier")
        this.throwError(
          new SyntaxError(
            "Expected string parameters inside of function declaration. " + arg,
          ),
        );
    ...
    const blocks: Record<string, Statement[]> = {
      body: [],
      public: [],
      private: [],
    };
    ...
    while (this.not_eof() && this.at().type !== TokenType.CloseBrace) {
      if (this.at().type === TokenType.Init)
        initFunction = this.parse_class_initializer();
      ...
      if (
        ...
        this.expect(
          TokenType.OpenBrace,
          `Expected '{' after ${currentBlock} block`,
        );
  ...
  private parse_class_member(): Statement {
    ...
    throw this.throwError(
      new ParserError(
        "Unexpected token in class body: " + this.at().value,
        this.at().type,
      ),
    );
  }
  ...
  private parse_if_statement(): Statement {
    ...
    if (!condition)
      throw this.throwError(
        new SyntaxError("Expected a valid condition for the 'if' statement"),
      );
    ...
    // Throws an error if the 'if' condition isn't followed by a '{' or a '=>' token
      ...
      this.throwError(
        new SyntaxError("Expected '{' of '=>' following 'if' body"),
      );
    ...
    if (this.at().type == TokenType.Else) {
      ...
      if (this.at().type == TokenType.If)
        elseBody.push(this.parse_if_statement());
      else {
        if (this.at().type == TokenType.OpenBrace) {
          // Eat the '{' token
          this.eat();
        ...
        // Throws an error if the 'else' condition isn't followed by a '{' or a '=>' token
          ...
          this.throwError(
            new SyntaxError("Expected '{' of '=>' following 'else' body"),
          );
      }
    }
  ...
  private parse_for_statement(): Statement {
    ...
    if (declarationToken.type == TokenType.Const)
      this.throwError(
        new SyntaxError(
          `Cannot reassign constant variable '${this.at().value}' inside a for loop.`,
        ),
      );
    ...
    this.expect(
      ...
      "Expected ')' following for compound assignment",
    );
    ...
    if (this.at().type == TokenType.OpenBrace) {
      ...
      this.throwError(
        new SyntaxError("Expected '{' of '=>' at the end of for statement"),
      );
  ...
  private parse_foreach_statement(): Statement {
    ...
    if (loopTypeToken.type == TokenType.Const)
      this.throwError(
        new SyntaxError(
          `Cannot reassign constant variable '${this.at().value}' inside a foreach loop.`,
        ),
      );
      ...
      this.expect(
        TokenType.Comma,
        "Expected ',' following foreach element declaration",
      );
      ...
      this.expect(
        ...
        "Expected ']' following foreach index declaration",
      );
      ...
      this.throwError(
        new SyntaxError(
          "Expected 'let', identifier or '[element, index]' following foreach keyword",
        ),
      );
    ...
    if (this.at().type == TokenType.OpenBrace) {
      ...
      this.throwError(
        new SyntaxError("Expected '{' of '=>' at the end of foreach statement"),
      );
  ...
  private parse_while_statement(): Statement {
    ...
    if (this.at().type == TokenType.OpenBrace) {
      ...
      this.throwError(
        new SyntaxError("Expected '{' of '=>' at the end of while statement"),
      );
  ...
  private async parse_import_statement(env: Environment) {
    ...
    if (this.at().type == TokenType.OpenBrace) {
      ...
      while (this.at().type != TokenType.CloseBrace) {
        if (this.at().type == TokenType.Comma)
          ...
          const imported = this.expect(
            TokenType.Identifier,
            "Expected identifier following import keyword",
          ).value;
      ...
      this.expect(
        TokenType.CloseBrace,
        "Expected '}' following import identifiers",
      );

      this.expect(
        TokenType.From,
        "Expected 'from' following import identifiers",
      );
    }
    const path = this.expect(
      TokenType.String,
      "Expected 'string' following import keyword",
    ).value;
    ...
    if (this.at().type == TokenType.As) {
      if (imports.length > 0)
        throw this.throwError(
          new ParserError(
            "Cannot use 'as' keyword when importing specific identifiers from a library",
            null,
          ),
        );

      this.eat();
      importName = this.expect(
        TokenType.Identifier,
        "Expected identifier following 'as' keyword in import statement",
      ).value;
    }

    if (this.env.imported.has(path))
      throw this.throwError(
        new ImportError(
          `Cannot import library '${path}' (most likely due to a circular import)`,
          null,
        ),
      );
    ...
    for (const obj of library_objects) {
      if (imports.length == 0) {
        if (importName) this.env.declareVar(importName, obj.object, true);
        else this.env.declareVar(obj.name, obj.object, true);
      } else {
        for (const imported of imports) {
          const value = obj.object.properties.get(imported);
          if (!value)
            throw this.throwError(
              new ImportError(
                `Library '${path}' does not export '${imported}'`,
                null,
              ),
            );
          this.env.declareVar(imported, value, true);
        }
      }
    }
  }
  ...
  private parse_export_declaration(): Statement {
    ...
    switch (this.at().type) {
      ...
      default:
        throw this.throwError(
          new ParserError(
            "Invalid export declaration. Only constants, functions and classes can be exported",
          ),
        );
    }
  ...
  private parse_choose_statement(): Statement {
    ...
    if (this.at().type == TokenType.ArrowOperator) {
      ...
      tempVariable = {
        ...
        symbol: this.expect(
          TokenType.Identifier,
          "Expected identifier following '=>' in choose statement",
        ).value,
        line: this.currentLine,
        column: this.currentColumn,
      } as Identifier;
    ...
    while (this.at().type != TokenType.CloseBrace) {
      ...
      if (this.at().type == TokenType.Case) {
        ...
        if (conditions.length == 0)
          throw this.throwError(
            new ParserError(
              "Expected at least one condition following 'case' keyword",
            ),
          );
        ...
        while (
          ...
          this.at().type != TokenType.DefaultCase &&
        ...
        if (defaultCase)
          throw this.throwError(
            new ParserError(
              "Only one default case is allowed in a choose statement",
            ),
          );
        ...
        throw this.throwError(
          new ParserError(
            "Expected 'case' or 'default' following choose subject",
          ),
        );
      }
    }
  ...
  private parse_compound_assignment_expression(): Expression {
    ...
    if (this.at().type == TokenType.CompoundOperator) {
      ...
      if (operator == "++" || operator == "--")
        value = { kind: "NumericLiteral", value: 1 } as NumericLiteral;
  ...
  private parse_ternary_expression(): Expression {
    ...
    if (this.at().type == TokenType.QuestionMark) {
      ...
      this.expect(
        TokenType.Colon,
        "Expected ':' between ternary expressions values",
      );
  ...
  private parse_choose_expression(): Expression {
    if (
      this.at().type != TokenType.Choose &&
      this.at().type != TokenType.ChooseAll
    )
    ...
    while (this.at().type != TokenType.CloseBrace) {
      ...
      if (this.at().type == TokenType.Case) {
        ...
        if (!body)
          throw this.throwError(
            new ParserError(
              "Expected a valid expression following 'case' keyword",
            ),
          );
        ...
        if (!body)
          throw this.throwError(
            new ParserError(
              "Expected a valid expression following 'default' keyword",
            ),
          );
  ...
  private parse_object_expression(): Expression {
    if (this.at().type != TokenType.OpenBrace)
      return this.parse_list_expression();
    ...
    while (this.not_eof() && this.at().type != TokenType.CloseBrace) {
      ...
      if (
        ...
        this.throwError(
          new ParserError("Object key expected", TokenType.Identifier),
        );
      if (this.at().type == TokenType.Comma) {
        this.eat(); // Go past comma
  ...
  private parse_list_expression(): Expression {
    if (this.at().type != TokenType.OpenBracket)
      return this.parse_membership_expression();
  ...
  private parse_args(): Expression[] {
    ...
    const args =
      this.at().type == TokenType.CloseParen ? [] : this.parse_args_list();
  ...
  private parse_args_list(): Expression[] {
    ...
    while (this.at().type == TokenType.Comma && this.eat())
      args.push(this.parse_statement());
  ...
  private parse_member_expression(): Expression {
    ...
    while (
      ...
      if (computed)
        ...
        this.throwError(
          new SyntaxError(
            "Cannot use dot operator without an identifier on the right",
          ),
        );
  ...
  private parse_primary_expression(): Expression {
    ...
    switch (token) {
      ...
      case TokenType.EOF:
        throw this.throwError(
          new RangeError("Parser unexpectedly reached the end of file"),
        );

      default:
        throw this.throwError(
          new SyntaxError(
            `Unexpected token found during parsing: ${JSON.stringify(this.at().value)} (${TokenType[this.at().type]})`,
          ),
        );
    }
  }
