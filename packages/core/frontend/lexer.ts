...
const isAlpha = (char: string): boolean =>
  char.toUpperCase() != char.toLocaleLowerCase() || char[0] == "_";
...
const isNegative = (src: string[], token: Token): boolean =>
  src[0] == "-" &&
  isNum(src[1]) &&
...
const isMultiCharToken = (
  src: string[],
  length: number,
  record: Record<string, TokenType>,
): boolean => src.slice(0, length).join("") in record;
...
function parseNumber(src: string[], token: Token): string {
  ...
  while (
    src.length > 0 &&
    (isNum(src[0]) || src[0] == "." || isNegative(src, token)) &&
    (src[0] != "-" || (num.length == 0 && src[0] == "-"))
  ) {
    ...
    if (src[0] == "." && num.includes("."))
      throw handleError(
        new LexerError("Invalid number format"),
        currentLine,
        currentColumn,
      );
...
function parseIdentifierOrKeyword(src: string[]): Token {
  ...
  while (src.length > 0 && (isAlpha(src[0]) || isNum(src[0])))
    ident += src.shift();
...
export function tokenize(sourceCode: string): Token[] {
  ...
  while (src.length > 0) {
    ...
    else if (isMultiCharToken(src, 3, tripleCharTokens))
      tokens.push(parseMultiCharToken(src, 3, tripleCharTokens));
    else if (isMultiCharToken(src, 2, doubleCharTokens))
      tokens.push(parseMultiCharToken(src, 2, doubleCharTokens));
    else if (isNum(current))
      tokens.push(
        token(parseNumber(src, tokens[tokens.length - 1]), TokenType.Number),
      );
    else if (isNegative(src, tokens[tokens.length - 1]))
      tokens.push(
        token(parseNumber(src, tokens[tokens.length - 1]), TokenType.Number),
      );
      ...
      throw handleError(
        new LexerError(
          "Unrecognized character found in source: " +
            JSON.stringify(current).charCodeAt(0),
        ),
        currentLine,
        currentColumn,
      );
