...
async function updateInterpreterPath(value: string) {
    ...
    let selectInterpreter = vscode.commands.registerCommand("gh3sp.selectInterpreter", async () => {
        ...
        fs.readdir(defaultPath, { withFileTypes: true }, (err, files) => {
            if (err || files.length === 0) {
                ...
                const versionChoices = versionFolders.map(
                    (folder) =>
                        ({
                            label: folder.name,
                    ...
                    .then((selected) => {
                        if (selected) {
                            if (selected.label === "Seleziona il percorso dell'interprete...") {
                                ...
                                updateInterpreterPath(selected.description as string); // Salva il percorso selezionato
                            }
                        }
                    });
            }
